...
class IncrementalFileProcessor:
    ...
    def _get_client(self, minio_config):
        if self.parsed_url.scheme in [
            "s3",
            "s3a",
        ] or self.checkpoint_parsed_url.scheme in ["s3", "s3a"]:
            if minio_config:
                return boto3.client(
                    "s3",
...
class DataTransformer:
    ...
    def transform_and_export(
        ...
        self.conn.execute(
            f"CREATE OR REPLACE VIEW transformed_data AS {transform_sql};"
        )
    ...
    for table_config in config["input"]["tables"]:
        ...
        if input_mode == "full":
            ...
            processor = IncrementalFileProcessor(
                input_path, f"{table_name}_checkpoint.json"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process data using a configuration file."
    )
    parser.add_argument(
